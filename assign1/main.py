...
class Block:
    ...
    def block_balance(self):
        """Net output of trxs in the current block."""
...
def _receive_transaction(time, message: Message):
    ...
    for neigh in graph[message.dest]:
        if message.source != neigh:
            ...
            heappush(q, (rec_time, _receive_transaction, Message(message.trx, message.dest, neigh)))
...
def _receive_block(time, msg: BlockMessage):
    ...
    if msg.block in block_book[msg.dest]:
        return
    logger.info(f"{time}s:\tBlock received: {msg}")
    ...
    for neigh in graph[msg.dest]:
        if msg.source != neigh:
            ...
            rec_time = time + latency
            heappush(q, (rec_time, _receive_block, BlockMessage(msg.block, msg.dest, neigh)))


#status = dict()
def _start_mining(time, generator):
    """Aggregate some transactions and schudule a _mine event for the generated block."""
    logger.info(f"{time}s:\tMining started by {generator}")
    ...
    pointer = new_block
    # if all("transact" not in str(k) for k in q):
        # for _ in range(5):
            # if not pointer or pointer.trxs:
                # break
            # pointer = pointer.prev
        # else:
            # logger.info(f"{time}s:\t Miner {generator} aborted mining due to stop condition")
...
def _mine(time, block: Block):
    ...
    if block.prev == curr_deep:  # Update longest chain
        ...
        logger.info(f"{time}s:\tDiscarded unmined old block: {block}")
    heappush(q, (time + 0.000000001, _start_mining, block.generator))



    #sched_minings = np.random.exponential(EXPO_MEAN, (NUM_PEERS,))
