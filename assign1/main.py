...
def _start_mining(time, generator):
    ...
    new_block_trxs = list()
    while pool[generator]:
        trx = pool[generator].pop()
...
def main():
    ...
    flag = False
    while q:
        if all('_mine' in str(k) for k in q):
            if flag and all(k==deepest_node[0] for k in deepest_node.values()) and len(pool[0]) == len(deepest_node[0].get_transaction_set()):
                logger.info(f"Only {len(q)} mining events scheduled now; exiting.")
                break
            flag = True
        execute(*heappop(q))
    print(block_book)
    for i, k in deepest_node.items():
        print(k)
